...
namespace EncryptedMessaging
{
    ...
    public class Context
    {
        ...
        /// <param name="getSecureKeyValue"></param>
        ...
        /// <param name="onMessageDelivered"></param>
        /// <param name="setSecureKeyValue"></param>
        ...
        public Context(Action<Action> invokeOnMainThread, Messaging.ViewMessageUi viewMessage, Action<Message> onContactEvent, Messaging.OnMessageArrived onNotification, Action<Contact, ulong, DateTime> onLastReadedTimeChange, Action<Contact, DateTime, bool> onMessageDelivered, SecureStorage.Initializer.SetKeyKalueSecure setSecureKeyValue, Func<string, string> getSecureKeyValue, string entryPoint, string networkName = "testnet", Contact.RuntimePlatform runtimePlatform = Contact.RuntimePlatform.Undefined, bool multipleChatModes = false, string privateKeyOrPassphrase = null, bool isServer = false, string CloudPath = null)
        {
#if DEBUG
            ...
            SecureStorage = new SecureStorage.Initializer(_instances.ToString(), setSecureKeyValue, getSecureKeyValue);
            ...
            // Restore the contacts list if a client is initialized with a new private key
            if (!string.IsNullOrEmpty(privateKeyOrPassphrase))
                IsRestored = true;
            ConnectivityChangeEventCollection.Add((bool connectivity) =>
            {
                if (connectivity && FirstConnectivityOn == false)
                {
                    FirstConnectivityOn = true;
                    OnInitializedAndConnectivityIsOn();
                }
            });
            Completed = true;
        }
        ...
        private bool FirstConnectivityOn;
        private void OnInitializedAndConnectivityIsOn()
        {
            if (IsServer) return;
            if (IsRestored)
            ...
            // var fromLastLoad = DateTime.UtcNow - Contacts.LastLoad;
            // if (fromLastLoad.TotalDays >= 1) // Check for avatar updates only once a day to not create too much cloud traffic
            //  Contacts.ForEachContact(contact =>
            //  {
            //   if ((DateTime.UtcNow - contact.LastMessageTime).TotalDays <= 7) // In order not to create traffic on the server, only the avatars of the recently used contacts will be updated
            //    contact.RequestAvatarUpdate();
            //  });
            // Contacts.LastLoad = DateTime.UtcNow;
            //}
        }
        ...
        internal static bool InternetAccess
        {
            ...
            set { _internetAccess = value; Channell.InternetAccess = value; }
        }
