...
namespace EncryptedMessaging
{
	...
	{
		public Contacts(Context context, Action<Contact, ulong, DateTime> onLastReadedTimeChange, Action<Contact, DateTime, bool> onMessageDelivered)
		{
			_context = context;
			OnLastReadedTimeChange = onLastReadedTimeChange;
		...
		internal Action<Contact, ulong, DateTime> OnLastReadedTimeChange;
		internal Action<Contact, DateTime, bool> OnMessageDelivered;

		public void RestoreContactFromCloud()
		{
			// the response are processed in class ProcessResponsesFromCloud
			Cloud.SendCloudCommands.GetObject(_context, "String", "MyName");
			Cloud.SendCloudCommands.GetAllObject(_context, "Contact");
		}
		...
		/// </summary>
		/// <param name="save">Save the changes and refresh the user interface</param>
		...
		internal void LoadContacts(bool onlyServer = false)
		{
			if (!_context.My.IsServer && !onlyServer)
			{
				...
				var objects = _context.SecureStorage.ObjectStorage.GetAllObjects(typeof(Contact));
				foreach (var obj in objects)
				{
					var contact = (Contact)obj;
					contact.Initialize(_context);
				...
				ForEachContact(contact =>
				{
					contact.ReadPosts();
						...
						Context.InvokeOnMainThread(() => OnLastReadedTimeChange(contact, readed.IdParticipant, readed.DateTime));
				});
				...
				if (ContactsList.ToList().Find((x) => x.Name == "Random User") == null)
					ContactsList.Add(new Contact(_context, participants: new List<byte[]>() { new CryptoServiceProvider().ExportCspBlob(false) }, "Random User"));
				if (_context.ContactConverter.PublicKeysToParticipants("An7tQNorwxKrg7H9wseMShCTl79hSH5g8wy+njNvpSrP", out List<byte[]> testUser))
				{
					var contact = new Contact(_context, testUser, "Test User");
					if (ContactsList.ToList().Find((x) => x.UserId == contact.UserId) == null)
						ContactsList.Add(contact);
				}
#endif
#if DEBUG_A
				if (_context.ContactConverter.PublicKeysToParticipants(Context.PubK_B, out List<byte[]> pubK_B))
				{
					var contact = new Contact(_context, pubK_B, "User B");
					if (ContactsList.ToList().Find((x) => x.UserId == contact.UserId) == null)
						ContactsList.Add(contact);
				}
#endif
#if DEBUG_B
				if (_context.ContactConverter.PublicKeysToParticipants(Context.PubK_A, out List<byte[]> pubK_A))
				{
		...
		internal DateTime LastLoad
		{
			get
			{
				...
				{
					var lastLoad = _context.SecureStorage.ObjectStorage.LoadObject(typeof(DateTime), "LastLoad");
					if (lastLoad != null)
						_lastLoad = (DateTime)lastLoad;
				}
				return _lastLoad;
			}
			set { _context.SecureStorage.ObjectStorage.SaveObject(value, "LastLoad"); }
		}
		...
		{
			get => _hibernatedChat;
			set { _hibernatedChat = value; _context.SecureStorage.ObjectStorage.SaveObject(_hibernatedChat, "HibernatedChat"); }
		}
		...
		/// <param name="sendMyContact">Specify if you want to send your contact (if individual), or the group, to the contact himself</param>
		...
		private bool AddContact(Contact contact, SendMyContact sendMyContact = SendMyContact.Send, bool sendToGroup = true, ContactMessage setSubContact = null)
		{
			...
			if (!contact.IsGroup)
			{
				if (sendMyContact != SendMyContact.None)
					_context.Messaging.SendContact(GetMyContact(sendMyContact == SendMyContact.SendNamelessForUpdate), contact, purposeIsUpdateOnly: purposeIsUpdateOnly);
			}
			else
			{
				if (sendToGroup)
					_context.Messaging.SendContact(contact, contact); // Send this group to all group members
				...
				foreach (var participant in participants)
					if (!participant.SequenceEqual(myKey)) // exclude myself
					{
						...
						if (sendToGroup)
							_context.Messaging.SendContact(contact, subContact); // All group members receive this contact
					}
			}
			return true;
		}
		...
		{
			List<byte[]> participants = contactMessage.GetParticipantsKeys(_context);
			contactMessage.GetProperties(_context, out var language, out Contact.RuntimePlatform os, out var firebaseToken, out var deviceToken);
			...
			if (string.IsNullOrEmpty(name) && contactMessage.Participants.Count <= 2)
				foreach (var participant in contactMessage.Participants)
					if (!participant.Key.SequenceEqual(_context.My.GetPublicKeyBinary()))
						name = participant.Name;
			Contact duplicate = ContactAlreadyExists(participants, name);
			if (contactMessage.Participants.Count == 1 && contactMessage.Participants[0].Key.SequenceEqual(_context.My.GetPublicKeyBinary())) // Prevent adding your contact
				return null;
			...
			var contact = new Contact(_context, participants, name, language, os, firebaseToken, deviceToken);
			...
			if (!string.IsNullOrEmpty(contactMessage.OldName) && contactMessage.OldName != contactMessage.Name)
				_context.Messaging.NotifyContactNameChange(contact); // I send the name with which the contract is registered in my address book, so I can have notifications with the name used locally in my contacts
			return contact;
		}
		...
		{
			if (!_context.ContactConverter.PublicKeysToParticipants(publicKeys, out List<byte[]> participants))
				return null;
			Contact duplicate = ContactAlreadyExists(participants, name);
			if (duplicate != null)
			{
			...
			var newContact = new Contact(_context, participants, name, isServer: isServer);
		...
		{
			if (!_context.ContactConverter.PublicKeysToParticipants(publicKeys, out List<byte[]> participants) || !_context.ContactConverter.ValidateKeys(publicKeys))
				return null;
			Contact duplicate = ContactAlreadyExists(participants, name);
			if (duplicate != null)
			{
			...
			var contact = new Contact(_context, participants, name, isServer: isServer);
		...
		public Contact AddContact(List<byte[]> participants, string name = null, bool isServer = false, string language = null, SendMyContact sendMyContact = SendMyContact.Send)
		{
			...
			var contact = new Contact(_context, participants, name, language, isServer: isServer);
		...
		public void ClearContact(string key)
		{
			...
			if (contact != null)
			{
				_context.Repository.ClearPosts(contact);
		...
		public Contact ContactAlreadyExists(List<byte[]> participants, string name)
		{
			...
			_context.ContactConverter.NormalizeParticipants(ref participantsClone);
		...
		/// <returns>Returns the duplicate contact if it already exists, otherwise null</returns>
		public Contact ContactAlreadyExists(string publicKeys, string name) => _context.ContactConverter.PublicKeysToParticipants(publicKeys, out List<byte[]> participants) ? ContactAlreadyExists(participants, name) : null;

		public string Pseudonym(string publicKeys) => _context.ContactConverter.PublicKeysToParticipants(publicKeys, out List<byte[]> participants) ? Pseudonym(participants) : null;

		public string Pseudonym(List<byte[]> participants) => Pseudonym(ParticipantsToUserIds(participants, _context));
		...
		public void Colors(List<byte[]> participants, out System.Drawing.Color light, out System.Drawing.Color dark) => Colors(ParticipantsToUserIds(participants, _context), out light, out dark);
		...
		{
			return nameless ? _context.My.CreateMyContact(nameless) : _context.My.Contact;
		}
		...
		public Contact GetCloudContact()
		{
			if (_cloud == null)
			{
				_context.ContactConverter.PublicKeysToParticipants(CloudPubKey, out List<byte[]> serverUser);
				_cloud = new Contact(_context, serverUser, "Server", "en", isServer: true);
			}
			return _cloud;
		}
	}
